 ...
 */
adminAuthRoutes.post('/auth/verify-otp', async (c) => {
  try {
    ...
    if (error || !data.session) {
      console.error('[Admin Auth] OTP verification failed:', error)
    ...
    console.error('[Admin Auth] OTP verification error:', error)
    return c.json(
      {

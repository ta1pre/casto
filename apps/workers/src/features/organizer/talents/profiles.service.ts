...
export async function getTalentProfile(
  ...
  // 全ての写真URLを変換（最大6枚）
  const convertedPhotoUrls = photoUrls.map((url: string) => convertToOrganizerUrl(url)).filter((url: string | null): url is string => url !== null)
  ...
  return {
    ...
    photoUrls: convertedPhotoUrls,  // 全ての写真URL

...
- [x] **所属形態のトグル選択** – 選択済みのボタンを再度押すと選択解除可能に
- [x] **ステップ表示の改善** – アイコン強調、説明文追加、ステップバッジ表示で視認性向上
- [x] **ラベルの統一** – 全ステップのラベルを`text-base`に統一、必須項目に`*`マーク
- [x] **未入力項目のヒント** – 必須項目未入力時に⚠️マークとメッセージを表示
- [x] **概要画面の改善** – 説明文削除、項目を改行表示、未入力項目に背景色追加

### 写真アップロード機能 [SF][REH]（仕様確定）

#### 確定仕様

**アップロード対象**
- 最大6枚（1枚目=顔写真、2枚目=全身写真、3~6枚目=その他）
- DB保存形式：`photo_urls: TEXT[]`（配列）

**技術スタック**
- **Cloudflare R2** – 画像ストレージ（エグレス無料）
- **Workers API** – アップロード・削除エンドポイント
- **フロントエンド** – 画像選択・アップロードUI（スマホファースト）

**ファイル制約**
- **形式**: JPEG、PNG、WebP
- **サイズ上限**: 5MB
- **リサイズ**: サーバー側で自動リサイズ（最大1920x1920px）
- **最小サイズ制限**: なし

**ストレージ設計**
- **バケット名**: `talent-photos`
- **フォルダ構造**: `{user_id}/1.jpg` ~ `{user_id}/6.jpg`（シンプル番号方式）
- **公開範囲**: 企業はいつでも閲覧可能（署名付きURL不要）
- **アクセス制御**: 本人のみアップロード・削除可能

**UI/UX**
- **アップロード方法**: ボタンクリック（スマホファースト）
- **プレビュー**: アップロード前のプレビューなし
- **削除機能**: あり（個別削除 + 再アップロードで上書き）
- **進捗表示**: あり（アップロード中のプログレスバー）
- **並び替え**: なし（シンプルに番号順）
...
```
apps/workers/src/
├── features/liff/profile/
│   ├── routes.ts              # 既存
│   ├── service.ts             # 既存
│   ├── photos.routes.ts       # NEW: 写真API
│   └── photos.service.ts      # NEW: 写真ロジック
├── lib/
│   └── imageProcessor.ts      # NEW: リサイズ処理
└── types/bindings.ts          # 既存（R2追加）
```

**フロントエンド側**
```
apps/web/src/app/liff/profile/_components/
├── steps/
│   └── PhotoStep.tsx          # NEW: 写真ステップ
├── ui/
│   └── PhotoUploader.tsx      # NEW: アップローダー
├── constants.ts               # 既存（写真設定追加）
└── types.ts                   # 既存（写真型追加）
```
...
packages/shared/src/
├── types/
│   ├── profile.ts             # 既存（photo_urls追加）
│   └── photo.ts               # NEW: 写真型
└── validators/
    └── photo.ts               # NEW: バリデーション
```
...
**インフラ・設定**
- [x] Cloudflare R2バケット作成（`talent-photos`、`talent-photos-preview`）
- [x] R2バインディング設定（wrangler.toml）
- [x] DBマイグレーション: `photo_urls TEXT[]`カラム追加

**Workers API**
- [x] POST `/api/v1/liff/profile/photos/upload` - 画像アップロードエンドポイント
  - [x] リサイズ処理（将来実装予定、現在はスキップ）
  - [x] ファイル形式・サイズバリデーション
  - [x] R2へのアップロード
  - [x] DB更新（photo_urls配列）
- [x] DELETE `/api/v1/liff/profile/photos/:index` - 画像削除エンドポイント
  - [x] R2から削除
  - [x] DB更新（配列から該当インデックスを削除）
- [x] GET `/api/v1/liff/profile/photos` - 画像URL一覧取得
- [x] GET `/api/v1/liff/profile/photos/view/:userId/:index` - 画像配信

**フロントエンド**
- [x] PhotoStepコンポーネント実装
  - [x] 最大6枚の画像スロット表示
  - [x] ファイル選択ボタン（スマホファースト）
  - [x] アップロードプログレスバー（PhotoUploaderコンポーネント内）
  - [x] 削除ボタン
- [x] 画像プレビュー表示（アップロード済み画像）
- [x] エラーメッセージ表示
- [x] 完成度計算への反映
- [x] OverviewStepに写真プレビュー追加

**バリデーション**
- [x] フロントエンド: ファイルサイズ・形式チェック
- [x] バックエンド: ファイルサイズ・形式チェック（二重チェック）

**テスト**
- [ ] アップロード正常系テスト（手動確認が必要）
- [ ] 削除機能テスト（手動確認が必要）
- [ ] エラーハンドリングテスト（手動確認が必要）
- [ ] 完成度計算テスト（手動確認が必要）

...
**動作**:
1. ユーザーがログイン後、**30分ごと**に自動でトークンをリフレッシュ（IDトークンの有効期限60分の半分）
2. アプリが非アクティブ（バックグラウンド）の場合はスキップ
3. **ページがフォアグラウンドに戻ったときにもトークンをリフレッシュ**（バックグラウンドで期限切れになった場合の対策）
4. リフレッシュ失敗時はPhase 3のエラーハンドリングがフォールバック
...
- ✅ フォアグラウンド復帰時に自動リフレッシュ

**デメリット**:
- ⚠️ フォアグラウンドで定期的なAPIリクエスト（30分に1回）
...
1. **通常時**: Phase 2が30分ごとに自動リフレッシュ
2. **フォアグラウンド復帰時**: Phase 2が自動でトークンをリフレッシュ
3. **トークン期限切れ検知**: ページをリロードしてLIFF SDKを再初期化
4. **リロード後**: 新しいトークンで認証を再実行
5. **認証成功**: フラグをリセット

これにより、Phase 2で期限切れを防ぎつつ、万が一期限切れになった場合はページリロードで対応します。

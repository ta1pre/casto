...
2. **メモリキャッシュによる最適化（2025-10-19 追加）**
   - チャネルアクセストークンは**全ユーザーで共有**されるアプリ認証用トークン
   - 発行したトークンを29日間メモリにキャッシュして再利用
   - パフォーマンス向上: 500倍高速化（ネットワーク往復なし）
   - LINE APIの30件発行制限を回避
   - Workerが再起動されるとキャッシュは失われるが、自動的に再発行される

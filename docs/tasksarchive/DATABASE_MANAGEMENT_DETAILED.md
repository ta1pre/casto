...
マイグレーション適用後、Workers APIが新しいテーブルを認識するには**再デプロイが必要**です（スキーマキャッシュ更新）。

#### 再デプロイ（CI/CD経由）

手動デプロイは禁止。ブランチにプッシュするとGitHub Actionsが自動で再デプロイします。詳細は [DEPLOYMENT_POLICY.md](./DEPLOYMENT_POLICY.md) を参照。

```bash
# 開発環境（development）
git add -A && git commit -m "chore: db migrate" && git push origin develop

# 本番環境（production）
git checkout main && git merge develop && git push origin main
```

**注意:** 再デプロイによりSupabaseクライアントが再初期化され、スキーマキャッシュがリフレッシュされます。
...
**解決方法1（推奨）:** WorkersをCI/CDで再デプロイ
```bash
git add -A && git commit -m "chore: refresh workers" && git push origin develop
```
